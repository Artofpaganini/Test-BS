...
## 0. Конвенции компонента

- **Булевы имена** — префикс `is`/`has`/`can` без исключений: `isOverlayBackground`, `isFillHeight`,
  `isSkipCollapsed`, `canPromoteForIme`. Флаги закрытия названы `is*Enabled`, а не `on*`: префикс
  `on` в Compose означает колбэк, для `Boolean` он врал бы о типе.
- **Порядок членов класса** — сверху вниз:
  `private val/var` → `val/var` → `override fun` → `public fun` → `internal/private fun` → `companion object`.
- **Вайринг снаружи** — только через изолированные `update*`-сеттеры, у каждой сущности свой,
  каждый с гардом (`==` для значений, `===` для ссылок). Поля под `private set`.
- **Идентификаторы слотов** `SubcomposeLayout` — строковые константы `UPPER_SNAKE`, не объекты-маркеры.
- **Вложенных функций нет** — только лямбды, поднятые в композицию (см. `detectBody`/`placeBody`).
- **`remember` — против аллокаций, не против арифметики.** Модификаторы и объекты кэшируем,
  вычитание двух `Int` — нет.
- **`derivedStateOf`** окупается, только когда дорогое вычисление читает часто меняющийся
  snapshot-state, а результат меняется редко. Для параметров-примитивов это чистый минус.
- **Логики в дата-классах нет** — производные величины считаются на месте использования.
...
scrim и контейнер, вайрит физику/IME/dismiss в стейт одним `SideEffect`'ом и обрабатывает
predictive-back.
...
| `onDismiss` | `rememberUpdatedState` БЕЗ делегата `by`: стейту нужен сам `State` — его идентичность стабильна, на ней стоит гард сеттера |
...
| `dimensions` | Все px-производные от плотности одним кэшируемым агрегатом (`rememberSheetDimensions`). Голых `toPx`/`toDp`/`roundToPx` в теле композиции нет |
| `isInteractionsEnable` | Жесты высоты отключены при `dragHandle == null` / Loading / открытой IME. `derivedStateOf` будит читателей только при смене значения |
...
| `widthModifier` | Под `remember`: `Modifier.width`/`fillMaxWidth` **аллокируют** элемент модификатора. Ключи — `dimensions.isWideScreen` и `config.maxWidth`, меняются только на смену размера экрана |
| `sheetBody` | Тело листа одной composable-лямбдой с параметром `isFillHeight`. Живёт в корне, потому что здесь под рукой `sheetScope` — привязка receiver'а слотов происходит прямо в аргументах `SheetBody`, без промежуточных обёрток |
| `detectBody` · `placeBody` | Два стабильных `@Composable () -> Unit` для двух слотов измерителя. Обязаны быть подняты в композицию: `SubcomposeLayout` сравнивает контент по ССЫЛКЕ, и лямбда, рождённая в measure-блоке, пересобирала бы всё тело листа каждый кадр драга |
| `additionalTopBody` | Карточка Additional Top с клипом верхних углов; сама зовёт `sheetScope.card()`. Отдельный слот над листом — её протрузия не входит в замер тела |

**Цвета.** Резолвятся не в корне, а прямо в аргументах: `config.colors.resolve*()` в вызовах
`SheetBody` и `SheetScrim`. Плата за это — `resolve*` внутри `sheetBody` исполняется в ОБЕИХ
подкомпозициях измерителя (detect и place), то есть тема читается дважды. Если стоимость чтения
темы вырастет, резолв надо поднять обратно в корень и передавать вниз готовые `Color`.
...
- `PredictiveBackHandler` — Android 14+: жест двигает лист за прогрессом через `backShift`;
  отмена -> 0, успех -> dismiss + сброс. `enabled = false` (дефолт `isBackPressEnabled`) пропускает
  событие хосту/Activity.
- `LaunchedEffect(state, containerSize.height, statusBarPx)` — поворот/resize: снап высоты к якорю.
...
- `SideEffect { … }` — единственная точка вайринга: конфиг закрытия, физика, scope и ссылка на
  ...
  с собственным гардом — после первого прохода записей нет. `isAlwaysFullScreenOnIme` = режим
  `StayUnderKeyboard` при наличии bottom-слота; стейт читает live-IME в `canPromoteForIme`
  без копий-снапшотов.
...
- Слоты биндятся к ОДНОМУ scope: detect/place-копии тела получают тот же `sheetScope`;
  `requestDismiss` из detect недостижим, потому что detect не размещается. Привязка живёт в корне,
  поэтому слой `layout/` про `XBottomSheetScope` ничего не знает — вниз уходят уже связанные лямбды.
...
Живые ручки (`isSkipCollapsed`/`peekFraction`/`anchors`) меняются прямо в композиции: покоящийся
...
| Поле | Зачем |
|---|---|
| `isSkipCollapsed` | Есть ли промежуточный Collapsed-якорь; `false` — открываем сразу в раскрытый стейт. Живая ручка |
...
| `isInitialLoading` | Стартовое значение `isLoading`, заданное билдером |
...
| `keyboardState` | Live-состояние IME (xbet `KeyboardLiftState`) — читается напрямую в `canPromoteForIme`, без копий-снапшотов |
| `isAlwaysFullScreenOnIme` | Конфиг-флаг `StayUnderKeyboard` + bottom-слот: bottom уходит под клавиатуру -> лист форсим в FullScreen |
| `isDismissOnSwipeDown` | Разрешён ли свайп-вниз-закрытие (settle добавляет Hidden-якорь на 0px) |
...
| `isFullScreen` | Стоит ли лист на якоре во весь экран. Вычисление живёт в стейте, а не в корне: его читают и корень, и контейнер, и тело |
...
|---|---|---|
| `updateDismissOnSwipeDown(value)` | Разрешение свайп-вниз-закрытия из `config.dismiss.isSwipeDownEnabled` | `==` |
...
| `onContentRemeasured()` | Контент вырос/подгрузился: высота следует за текущим якорем. `isSkipCollapsed` + `Content`, заполнивший экран (`isFillMode`) -> авто-FullScreen. В Loading — no-op (высоту ведут `show`/`markContentReady`) |
...
| Функция | Зачем |
|---|---|
| `openTarget(isSkipCollapsed)` | Стейт, в который лист открывается при `show`, по замеру и `isSkipCollapsed` |
| `expandTarget()` | Стейт раскрытия из Collapsed: контент в пределах экрана -> `ExpandedContent`, иначе `ExpandedFullScreen` |
| `anchorPx(value, isSkipCollapsed)` | Высота (px) якоря для заданного стейта по текущим метрикам |
...
отсортированы по высоте и distinct по px, БЕЗ Hidden: `isDismissOnSwipeDown` мутабелен, поэтому
...
раз и задаёт начальную вариацию; динамику (`peekFraction`/`anchors`/`isSkipCollapsed`) даёт не
...
- Живые поля (`peekFraction`/`anchors`/`isSkipCollapsed`) НЕ сохраняются: стейт пересоздаётся
...
кладутся в FIFO-канал стейта. Свой флаг называется `isEnabled` и уходит в чужой
`draggable(enabled = …)` — имя параметра Compose-API не переименовываем.
...
`isEnabledState` — тот же `derivedStateOf`, что гейтит `sheetDrag`; при видимой IME он `false`,
...
Чистый измеритель: `SubcomposeLayout` мерит тело ДВАЖДЫ — detect (wrap контента -> `contentHeightPx`)
и place (fixed по `offset` -> реальная высота). Готовые тела слотов приходят параметрами из корня,
поэтому визуальные параметры листа сюда не тянутся и слой ничего не знает про `XBottomSheetScope`.
`withAdjustmentForKeyboard` (подъём над IME) вешается только вне FullScreen: в FullScreen лист у
потолка, там Middle сжимается вместо подъёма.

| Место | Зачем |
|---|---|
| `shadowModifier` · `keyboardAdjustmentModifier` | Под `remember`: оба **аллокируют** элемент модификатора, ключи меняются редко. `remember` тут против аллокаций, а не против арифметики |
| `ceilingPx` | Считается внутри measure-блока: в композиции не нужен, слота не занимает |
...
| `SLOT_DETECT` · `SLOT_PLACE` · `SLOT_ADDITIONAL_TOP` | Строковые идентификаторы слотов `SubcomposeLayout` вместо объектов-маркеров: сравниваются по `equals`, читаются в отладке |
...
| `sizeModifier` | `isFillHeight` (place): тело заполняет `offset` — фон на всю высоту, нет дыры снизу. `!isFillHeight` (detect): wrap по контенту |
...
`@Immutable` заявлен явно, а не выводится компилятором. Только данные — производные величины
(`ceilingPx`) считаются на месте использования, в дата-классе логики нет. Инстанс собирается
внутри `rememberSheetDimensions`, поэтому на рекомпозиции не аллокируется.

### `layout/SheetDimensions.kt` · `layout/RememberSheetDimensions.kt`

Все величины, производные от плотности, в одном `@Immutable`-агрегате: `insets`,
`scrimFadeDistancePx`, `predictiveBackMaxShiftPx`, `isWideScreen`.

Зачем агрегат, а не четыре `val` в композиции:
- в теле composable-функции не остаётся голых `toPx`/`toDp`/`roundToPx` — весь `with(density) { }`
  собран в одном месте и исполняется только под `remember`;
- слот-таблица композиции это `Array<Any?>`, поэтому `remember` над примитивом **боксит** его
  (`Dp` — value class над `Float`, `Int`, `Float`). Поля ВНУТРИ объекта не боксятся: боксится
  один агрегат, а не четыре значения;
- один `remember` вместо четырёх — меньше слотов и меньше сравнений ключей.

Ключи — плотность, ширина и высота контейнера, инсеты и Dp-токены конфига. Пересборка только на
смену плотности, поворот и появление/уход системных баров.

`isWideScreen` хранится готовым флагом, а не промежуточным `screenWidthDp`: наружу нужен только
результат сравнения с порогом.

### `layout/SheetScrim.kt`

Полноэкранный scrim: тачи под листом гасятся ВСЕГДА (hit-таргет), затемнение опционально.
Тап вне листа -> закрытие при `isDismissOnOutsideTap`; тапы по самому листу сюда не долетают
...
**Общий `SnapshotFlowManager`.** Базовый `snapshotFlow { }` инстанцирует себе менеджер под капотом,
то есть четыре вызова дали бы четыре подписки на snapshot-систему. Оверлоад
`snapshotFlow(manager, block)` позволяет разделить один apply-observer на все четыре потока.
Условие безопасности из доков — не шарить менеджер между потоками, которые собирают параллельно;
наши коллекторы живут в `AndroidUiDispatcher.Main` и параллельно идти не могут, для такого случая
шаринг прямо рекомендован. API помечен `@ExperimentalComposeRuntimeApi` — при апгрейде Compose
сигнатуру надо перепроверять, откат тривиален (убрать первый аргумент у четырёх вызовов).

**Жизненный цикл менеджера.** Канон: `remember` для создания, `DisposableEffect` для
освобождения, `LaunchedEffect` для коллекторов — именно в таком порядке объявления. Порядок не
косметика: Compose дёргает `onForgotten` в ОБРАТНОМ порядке слотов, поэтому объявленный ниже
`LaunchedEffect` отменяет корутины ПЕРВЫМ, а `dispose()` из `DisposableEffect` выполняется после.
Переставь их местами — менеджер освободится раньше, чем остановятся коллекторы. `dispose()` строго
однократен: повторный вызов кидает `IllegalStateException`. Внешний менеджер поток не освобождает
сам («the flow does not dispose it, because other flows may still be using it»).

Ключи `LaunchedEffect` — `(manager, state, keyboardState)`. Одного `manager` мало: он из
`remember` без ключей, то есть стабилен навсегда, и эффект никогда не перезапустился бы при смене
стейта или источника IME.
...
Способы закрытия листа: тап вне листа, свайп вниз, системный back. Дефолт `isBackPressEnabled = false` —
...
## 8. Заимствованные API рантайма

Список экспериментальных и низкоуровневых вещей, за которыми надо следить при апгрейде Compose.

| API | Где | Риск |
|---|---|---|
| `SnapshotFlowManager` + `snapshotFlow(manager, block)` | `observe/ObserveSheetState.kt` | `@ExperimentalComposeRuntimeApi`. Появился в runtime 1.9.0, у нас 1.11.2. Откат — убрать первый аргумент у четырёх вызовов и снять `remember`/`DisposableEffect` |
| `SubcomposeLayout` | `layout/SheetContainer.kt` | Стабилен, но сравнение контента слота по ССЫЛКЕ — деталь реализации, на которой держится наша производительность драга |
| `withAdjustmentForKeyboard` · `withKeyboardShrink` · `KeyboardLiftState` | `org/xplatform/uikit/**` | Не наш код, verbatim-копия из xbet. Расхождение с оригиналом = боль при мердже |
