...
**(ПЕРЕСМОТРЕНО: DSL-конфиг.)** Скалярные конфиги листа собраны в `@Immutable data class XBottomSheetConfig`
(value-equality → инлайновый билдер на call-site не ломает skip рекомпозиции), а параметры стейта — в
`XBottomSheetStateConfig`; оба строятся DSL-билдерами (`@DslMarker XBottomSheetDsl`, дефолты только в билдерах,
БЕЗ require-валидаций — по решению юзера). Слоты (`additionalTop/top/bottom/middle`) остаются отдельными
параметрами (правило: никаких лямбд в data class). Ниже — актуальные сигнатуры:
...
fun XBottomSheet(
    ...
    config: XBottomSheetConfig = XBottomSheetConfig.Default,  // DSL: xBottomSheetConfig { ... }
    ...
    middle: @Composable () -> Unit,                  // единственная скроллируемая секция (скролл предоставляет контент)
)

// config = xBottomSheetConfig {
//     overlayBackground = true; dragHandle = DragHandleStyle.Theme; additionalTopCornerRadius = 0.dp
//     dismiss { onOutsideTap = true; onSwipeDown = true }
//     keyboard { bottomBehavior = BottomKeyboardBehavior.Lift }   // StayUnderKeyboard → bottom под клавиатуру
// }

enum class DragHandleStyle { Theme, Static }
enum class BottomKeyboardBehavior { Lift, StayUnderKeyboard }

@Composable
fun rememberXBottomSheetState(
    configure: XBottomSheetStateConfigBuilder.() -> Unit = {},   // { skipCollapsed = …; initialLoading = …;
): XBottomSheetState                                             //   peekFraction = …; anchors { anchor("half", 0.5f) } }
```
...
class XBottomSheetState internal constructor(val skipCollapsed: Boolean, initialLoading: Boolean) {
    ...
    suspend fun markContentReady() { isLoading = false; animateTo(metrics!!.openTarget(skipCollapsed)) }
...
LaunchedEffect(Unit) {
    ...
    sheet.markContentReady()                  // высота анимируется к целевому стейту
}
...
XBottomSheet(
    ...
    bottom = { PresetSingleButton("Показать все", onClick = ::showAll) },
) {
...
| Loading | `state.isLoading` / `markContentReady()` |
...
- [x] `XBottomSheetState.kt` — `@Stable class XBottomSheetState` + `rememberXBottomSheetState(...)`,
      публичное API `show/expand/hide/markContentReady`, internal `onContentRemeasured` (§4). Compose-state, не StateFlow.
...
- [x] `PresetTitle.kt` · `PresetBodyText.kt` · `PresetMenuCell.kt` · `PresetSingleButton.kt` · `PresetSearchField.kt`
...
- [x] (d) Loading→markContentReady: `initialLoading=true`, `show()`→Loading 192dp/Loader, `delay`→`markContentReady()`→анимация.
...
- [x] **Фаза A — State/Anchors.** `XBottomSheetDefaults`, `theme/XTheme`, `SheetAnchors`, `XBottomSheetState`.
      Критерий: `:app:assembleDebug` зелёный; API `rememberXBottomSheetState`/`show`/`expand`/`hide`/`markContentReady`
...
9. **markContentReady() и Loading-метрики** — во время Loading middle = Loader, поэтому `contentHeightPx` = высота
   Loader'а. `markContentReady()` снимает `isLoading`, ДОЖИДАЕТСЯ ре-замера реального контента (`snapshotFlow{metrics}`
