...
import com.onexui.bottomsheet.style.MAX_ADDITIONAL_TOP_PEEK
...
private fun saveState(state: XBottomSheetState): List<Any> {
    ...
    return listOf(
        ...
        style.additionalTop.peek.value,
...
private fun restoreState(saved: List<Any>): XBottomSheetState {
    ...
    val style = XBottomSheetStyle(
        ...
        additionalTop = AdditionalTopStyle(
            ...
            peek = reader.readFloat(MAX_ADDITIONAL_TOP_PEEK.value).dp.coerceIn(0.dp, MAX_ADDITIONAL_TOP_PEEK),
...
private fun anchorFromTag(tag: String): AnchorState = when {
    tag.startsWith(ANCHOR_FRACTION_PREFIX) ->
        AnchorState.Fraction(tag.removePrefix(ANCHOR_FRACTION_PREFIX).toFloatOrNull() ?: DEFAULT_ANCHOR_FRACTION)
...
private fun dragHandleTag(style: DragHandleStyle?): String = when (style) {
    is DragHandleStyle.Theme ->
        if (style.color.isSpecified) "$DRAG_HANDLE_THEME_PREFIX${colorToLong(style.color)}" else DRAG_HANDLE_THEME_TAG
    DragHandleStyle.Static -> DRAG_HANDLE_STATIC_TAG
...
private fun dragHandleFromTag(tag: String): DragHandleStyle? = when {
    ...
    tag.startsWith(DRAG_HANDLE_THEME_PREFIX) ->
        DragHandleStyle.Theme(
            colorFromLong(tag.removePrefix(DRAG_HANDLE_THEME_PREFIX).toLongOrNull() ?: colorToLong(Color.Unspecified)),
        )
    tag == DRAG_HANDLE_STATIC_TAG -> DragHandleStyle.Static
...
private const val DEFAULT_ANCHOR_FRACTION = 0.60f
...
private const val DRAG_HANDLE_THEME_PREFIX = "theme:"
private const val DRAG_HANDLE_STATIC_TAG = "static"
